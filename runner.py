#!/usr/bin/env python3
"""
Task executor — 阶段 1(并发与资源限额)。

在阶段 0 地基(SQLite 状态库 / 容器命名 / 独立日志 / 入库执行两步)之上,新增:
  - 可配置并发:线程池 + 真正限流,同时在飞的任务数受 --concurrency 限制
  - 资源限额:每个容器带 --memory / --memory-swap / --cpus
  - OOM 识别:内存超限被 OOM Kill(退出码 137 / State.OOMKilled)判为 failed,
    而不是拖垮宿主机
  - 并发安全:SQLite 写入加锁串行化
...
def run_task(conn, task_id, command, limits, log_dir=LOG_DIR):
    """在独立容器里执行一个任务(带资源限额),状态与输出分别落库、落日志文件。

    limits: dict(memory, cpus)。可被线程池并发调用,写库走 db_write 串行化。
    """
    ...
    oom = proc.returncode == 137 and was_oom_killed(name)
    ...
    tag = " [OOM-killed]" if oom else ""
    print(f"[task {task_id}] END: {state} (exit={proc.returncode}){tag}")
    return proc.returncode


def parse_args():
    p = argparse.ArgumentParser(description="容器任务执行器(阶段 1)")
...
def main():
    ...
    print(f"\n待执行 {len(rows)} 条任务,并发={args.concurrency},"
          f"限额 memory={args.memory} cpus={args.cpus}\n")
